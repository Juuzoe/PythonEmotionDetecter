...
def summarize(session: dict[str, np.ndarray]) -> dict[str, Any]:
    ...
    Only frames with a face contribute to affect, vitals and action-unit
    statistics: nothing measured in an empty frame is a measurement of anyone.
    """
    t = session["t"]
    n = len(t)
    if n == 0:
        return {"frames": 0, "duration_s": 0.0}
    face = np.nan_to_num(session["face"], nan=0.0) == 1
    labels = session["emotion"]
    with_face = [str(label) for label, f in zip(labels, face) if f and label]
    distribution = (
        {label: with_face.count(label) / len(with_face) for label in EMOTIONS} if with_face else {}
    )

    def on_face(column: str) -> np.ndarray:
        return session[column][face]

    hr = on_face("hr_bpm")
    quality = on_face("hr_quality").astype(str)
    reliable = np.isfinite(hr) & np.isin(quality, ["good", "fair"])
    aus = {
        code: float(np.nanmean(session[code][face]))
        for code in AU_CODES
        if code in session and np.isfinite(session[code][face]).any()
    }
    ...
    return {
        ...
        "dominant_emotion": (
            max(distribution, key=lambda k: distribution[k]) if distribution else None
        ),
        "valence": _stats(on_face("valence")),
        "arousal": _stats(on_face("arousal")),
        "inertia": _stats(on_face("inertia")),
        "switch_rate": _stats(on_face("switch_rate")),
        "heart_rate": _stats(hr[reliable]),
        "heart_rate_coverage": float(reliable.mean()) if len(reliable) else 0.0,
        "rmssd": _stats(on_face("rmssd_ms")),
        "breathing": _stats(on_face("breathing_bpm")),
        "blink_count": int(blink_counts.max()) if len(blink_counts) else 0,
        "blink_rate": _stats(on_face("blink_rate")),
        "perclos": _stats(on_face("perclos")),
...
def render_figures(session: dict[str, np.ndarray], out_dir: Path) -> list[Path]:
    ...
    Figures are built with the object-oriented API inside an ``rc_context``, so
    calling this from a notebook or script leaves the caller's matplotlib
    backend and style untouched.
    """
    try:
        import matplotlib
    ...
    def finite_max(values: np.ndarray, floor: float) -> float:
        finite = values[np.isfinite(values)]
        return max(floor, float(finite.max()) * 1.2) if len(finite) else floor

    with matplotlib.rc_context(cast(Any, rc)):
        # 1. Emotion probabilities over time (stacked area, fixed order and colours).
        fig = Figure(figsize=(10, 3.6), layout="constrained")
        ax = fig.subplots()
        ...
        fig = Figure(figsize=(6.4, 6), layout="constrained")
        ax = fig.subplots()
        ax.add_patch(Circle((0, 0), 1.0, fill=False, color=c["axis"], linewidth=1.0))
        ...
        if ok.any():
            ...
            scatter = ax.scatter(
                v[ok], a[ok], c=ts[ok], cmap=cmap, s=12, edgecolors="none", zorder=2
            )
        ...
        fig = Figure(figsize=(10, 6.8), layout="constrained")
        ...
        axes[1].set_ylim(0, finite_max(blink_rate, RESTING_BLINK_RATE * 1.3))
        perclos = session["perclos"][sl] * 100.0
        axes[2].plot(ts, perclos, color=blue, linewidth=2.0)
        axes[2].set_ylim(0, finite_max(perclos, 5.0))
        ...
        if codes:
            ...
            fig = Figure(figsize=(10, 0.28 * len(codes) + 1.6), layout="constrained")
            ax = fig.subplots()
            image = ax.imshow(
                matrix,
                aspect="auto",
...
def build_report(
    ...
    target = Path(out_dir) if out_dir else session_path.parent / (session_path.stem + "_report")
